...
<img src="p8105_hw6_kb3246_files/figure-gfm/model1 plot-1.png" width="95%" />
...
birthweight_cv = crossv_mc(birthweight, 100) %>% 
  ...
  mutate(rmse_m1 = map2_dbl(model1_cv, test, ~rmse(model = .x, data = .y)),
         rmse_m2 = map2_dbl(model2_cv, test, ~rmse(model = .x, data = .y)), 
         rmse_m3 = map2_dbl(model3_cv, test, ~rmse(model = .x, data = .y)))
```
